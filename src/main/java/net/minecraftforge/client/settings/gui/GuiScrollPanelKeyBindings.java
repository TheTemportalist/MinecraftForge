...
class GuiScrollPanelKeyBindings extends GuiScrollPanel<KeyBinding>
{
            ...
            int mouseEvent, int relativeX, int relativeY)
    {
        if (this.selectedIndex >= 0)
        {
            ...
            this.onBindingChanged(keyBinding);
    ...
    {
        if (this.selectedIndex >= 0 && !KeyModifier.isKeyCodeModifier(keyCode))
        {
            KeyBinding keyBinding = this.getElement(this.selectedIndex);
    ...
    @Override
    protected void drawEntry(int index, KeyBinding entry, int x, int y, int listWidth,
            int slotHeight,
    ...
    private String getButtonName(int index, KeyBinding keyBinding)
    {
        ...
        { // if is changing
            return TextFormatting.WHITE + "> " + TextFormatting.YELLOW + display
